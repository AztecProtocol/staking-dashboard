locals {
  create_dns_record = var.env == "prod" ? true : false
}
terraform {
...
resource "aws_cloudfront_distribution" "staking_dashboard_distribution" {
  ...
  aliases = var.env == "prod" ? ["stake.aztec.network"] : []
...
module "domain" {
  ...
  domain_name               = var.env == "prod" ? "stake.aztec.network" : "stake.${var.env}.aztec.network"
