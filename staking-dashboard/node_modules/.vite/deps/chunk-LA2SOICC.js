...
import {
  formatAbiItem,
  parseAbiItem
} from "./chunk-Q6EHPFDT.js";
import {
  createBase58check
} from "./chunk-3BHFJ7RT.js";
import {
  createClient,
  createNonceManager,
  encode,
...
import {
  ...
  size2 as size,
  slice2 as slice
} from "./chunk-IH4OW7UQ.js";
import {
  AccountNotFoundError,
  formatLog,
  formatTransactionReceipt,
  sha256 as sha2562
} from "./chunk-VEQIG2YB.js";
import {
  BaseError,
...
function getClientDataJSON(options) {
  const { challenge: challenge3, crossOrigin = false, extraClientData, origin = window.location.origin } = options;
  return JSON.stringify({
    type: "webauthn.get",
    challenge: fromHex4(challenge3, { url: true, pad: false }),
    origin,
    crossOrigin,
    ...extraClientData
  });
...
function getCredentialCreationOptions(options) {
  ...
  }, challenge: challenge3 = createChallenge, excludeCredentialIds, name: name_, rp = {
...
function getSignPayload2(options) {
  const { challenge: challenge3, crossOrigin, extraClientData, flag, origin, rpId, signCount, userVerification = "required" } = options;
...
function verify2(options) {
  const { challenge: challenge3, hash: hash2 = true, metadata, publicKey, signature } = options;
  ...
  if (fromBytes(toBytes4(challenge_extracted)) !== challenge3)
    return false;
...
var _2n3 = BigInt(2);
var _3n2 = BigInt(3);
function calcFrobeniusCoefficients(Fp3, nonResidue, modulus, degree, num3 = 1, divisor) {
...
var bitMask2 = (n) => (_2n5 << BigInt(n - 1)) - _1n5;
...
function tonelliShanks(P) {
  const legendreC = (P - _1n6) / _2n6;
  let Q, S, Z;
  for (Q = P - _1n6, S = 0; Q % _2n6 === _0n6; Q /= _2n6, S++)
    ;
  for (Z = _2n6; Z < P && pow(Z, legendreC, P) !== P - _1n6; Z++) {
    if (Z > 1e3)
  ...
  return function tonelliSlow(Fp3, n) {
    ...
    while (!Fp3.eql(b, Fp3.ONE)) {
      ...
      const ge = Fp3.pow(g, _1n6 << BigInt(r - m - 1));
      g = Fp3.sqr(ge);
      x = Fp3.mul(x, ge);
      b = Fp3.mul(b, g);
      r = m;
...
function FpInvertBatch2(f2, nums) {
  ...
  const lastMultiplied = nums.reduce((acc, num3, i) => {
    if (f2.is0(num3))
      return acc;
    tmp[i] = acc;
...
function Field2(ORDER, bitLen3, isLE3 = false, redef = {}) {
  ...
  const f2 = Object.freeze({
    ...
    pow: (num3, power) => FpPow2(f2, num3, power),
    ...
    cmov: (a, b, c) => c ? b : a,
    toBytes: (num3) => isLE3 ? numberToBytesLE(num3, BYTES) : numberToBytesBE2(num3, BYTES),
    fromBytes: (bytes) => {
      if (bytes.length !== BYTES)
        throw new Error("Field.fromBytes: expected " + BYTES + " bytes, got " + bytes.length);
      return isLE3 ? bytesToNumberLE(bytes) : bytesToNumberBE2(bytes);
    }
  });
...
function mapHashToField2(key, fieldOrder, isLE3 = false) {
  ...
  const num3 = isLE3 ? bytesToNumberLE(key) : bytesToNumberBE2(key);
...
function weierstrassPoints2(opts) {
  ...
  function normPrivateKeyToScalar(key) {
    ...
    if (wrapPrivateKey)
      num3 = mod2(num3, N);
    aInRange("private key", num3, _1n8, N);
    return num3;
  }
...
function weierstrass(curveDef) {
  ...
  const numToNByteStr = (num3) => bytesToHex2(numberToBytesBE2(num3, CURVE.nByteLength));
  ...
  const slcNum = (b, from10, to) => bytesToNumberBE2(b.slice(from10, to));
  class Signature {
  ...
  function int2octets(num3) {
    aInRange("num < 2^" + CURVE.nBitLength, num3, _0n8, ORDER_MASK);
...
function taggedHash2(tag, ...messages) {
  ...
  return sha2564(concatBytes3(tagP, ...messages));
}
...
var htf = (() => createHasher2(secp256k12.ProjectivePoint, (scalars) => {
...
*/
//# sourceMappingURL=chunk-LA2SOICC.js.map
