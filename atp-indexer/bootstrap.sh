...
if [ "$ENVIRONMENT" != "sepolia" ] && [ "$ENVIRONMENT" != "staging" ] && [ "$ENVIRONMENT" != "prod" ]; then
  echo "Error: Environment must be 'sepolia' or 'staging' or 'prod'"
  exit 1
fi
...
function deploy() {
  ...
  if [ "$infra_environment" = "dev" ]; then
    if [ -z "${SEPOLIA_RPC_URL:-}" ]; then
      echo "Error: SEPOLIA_RPC_URL must be set"
      exit 1
    fi
    RPC_URL=$SEPOLIA_RPC_URL
    CHAIN_ID=11155111
    chain_environment="sepolia"
    infra_parent_environment="dev"
  fi

  if [ "$infra_environment" = "dev-tn" ]; then
  ...
  if [ "$infra_environment" = "staging" ]; then
    if [ -z "${RPC_URL:-}" ]; then
    ...
    RPC_URL=$RPC_URL
    CHAIN_ID=1
    chain_environment="staging"
    infra_parent_environment="staging"
  fi
